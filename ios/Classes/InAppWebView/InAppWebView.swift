...
public class InAppWebView: WKWebView, UIScrollViewDelegate, WKUIDelegate, WKNavigationDelegate, WKScriptMessageHandler, UIGestureRecognizerDelegate, PullToRefreshDelegate {
    ...
    private func gestureRecognizerWithDescriptionFragment(_ descriptionFragment: String) -> UILongPressGestureRecognizer? {
        let result = self.scrollView.subviews.compactMap({
    ...
    @objc func longPressGestureDetected(_ sender: UIGestureRecognizer) {
        ...
        evaluateJavaScript("window.\(JAVASCRIPT_BRIDGE_NAME)._findElementsAtPoint(\(touchLocation.x),\(touchLocation.y))", completionHandler: {
            (value, error) in
            if error != nil {
    ...
    public override func canPerformAction(_ action: Selector, withSender sender: Any?) -> Bool {
        if let _ = sender as? UIMenuController {
            ...
            if contextMenuIsShowing, !action.description.starts(with: "onContextMenuActionItemClicked-") {
                let id = action.description.compactMap({
                    $0.asciiValue?.description
                })
                .joined()
                let arguments: [String: Any?] = [
                    "iosId": id,
    ...
    @objc func onCreateContextMenu() {
        let mapSorted = SharedLastTouchPointTimestamp.sorted {
            $0.value > $1.value
        }
        ...
        if let lastLongPressTouhLocation = lastLongPressTouchPoint {
            if configuration.preferences.javaScriptEnabled {
                self.evaluateJavaScript("window.\(JAVASCRIPT_BRIDGE_NAME)._findElementsAtPoint(\(lastLongPressTouhLocation.x),\(lastLongPressTouhLocation.y))", completionHandler: {
                    (value, error) in
                    if error != nil {
                        print("Long press gesture recognizer error: \(error?.localizedDescription ?? "")")
                    }
    ...
    public func takeScreenshot (with: [String: Any?]?, completionHandler: @escaping (_ screenshot: Data?) -> Void) {
        ...
        takeSnapshot(with: snapshotConfiguration, completionHandler: {
            (image, error) -> Void in
    ...
    public func createPdf (configuration: [String: Any?]?, completionHandler: @escaping (_ pdf: Data?) -> Void) {
        ...
        createPDF(configuration: pdfConfiguration) {
            (result) in
            switch (result) {
    ...
    public func saveWebArchive (filePath: String, autoname: Bool, completionHandler: @escaping (_ path: String?) -> Void) {
        createWebArchiveData(dataCompletionHandler: {
            (webArchiveData) in
    ...
    public func enablePluginScriptAtRuntime(flagVariable: String, enable: Bool, pluginScript: PluginScript) {
        evaluateJavascript(source: flagVariable) {
            (alreadyLoaded) in
    ...
    public func injectDeferredObject(source: String, contentWorld: WKContentWorld, withWrapper jsWrapper: String?, completionHandler: ((Any?) -> Void)? = nil) {
        ...
        evaluateJavaScript(jsToInject, frame: nil, contentWorld: contentWorld) {
            (evalResult) in
            guard let completionHandler = completionHandler else {
                return
            }
    ...
    public func webView(_ webView: WKWebView,
        ...
        self.webView(webView, decidePolicyFor: navigationAction, decisionHandler: {
            (navigationActionPolicy) -> Void in
    ...
    public func webView(_ webView: WKWebView,
        ...
        if navigationAction.request.url != nil {
            if let useShouldOverrideUrlLoading = options?.useShouldOverrideUrlLoading, useShouldOverrideUrlLoading {
                shouldOverrideUrlLoading(navigationAction: navigationAction, result: {
                    (result) -> Void in
    ...
    public func webView(_ webView: WKWebView,
        ...
        if useOnNavigationResponse != nil, useOnNavigationResponse! {
            onNavigationResponse(navigationResponse: navigationResponse, result: {
                (result) -> Void in
                if result is FlutterError {
                    print((result as! FlutterError).message ?? "")
                    decisionHandler(.allow)
    ...
    public func webView(_ webView: WKWebView, didReceive challenge: URLAuthenticationChallenge, completionHandler: @escaping (URLSession.AuthChallengeDisposition, URLCredential?) -> Void) {
        ...
        challenge.protectionSpace.authenticationMethod == NSURLAuthenticationMethodNTLM {
            ...
            onReceivedHttpAuthRequest(challenge: challenge, result: {
                (result) -> Void in
                if result is FlutterError {
                    print((result as! FlutterError).message ?? "")
            ...
            onReceivedServerTrustAuthRequest(challenge: challenge, result: {
                (result) -> Void in
                if result is FlutterError {
                    print((result as! FlutterError).message ?? "")
                    completionHandler(.performDefaultHandling, nil)
            ...
            onReceivedClientCertRequest(challenge: challenge, result: {
                (result) -> Void in
                if result is FlutterError {
                    print((result as! FlutterError).message ?? "")
                    completionHandler(.performDefaultHandling, nil)
    ...
    public func webView(_ webView: WKWebView, runJavaScriptAlertPanelWithMessage message: String,
        ...
        onJsAlert(frame: frame, message: message, result: {
            (result) -> Void in
            if result is FlutterError {
                print((result as! FlutterError).message ?? "")
                completionHandler()
    ...
    func createConfirmDialog(message: String?, responseMessage: String?, confirmButtonTitle: String?, cancelButtonTitle: String?, completionHandler: @escaping (Bool) -> Void) {
        ...
        confirmController.addAction(UIAlertAction(title: okButton, style: .default, handler: {
            (action) in
            completionHandler(true)
        }))
        confirmController.addAction(UIAlertAction(title: cancelButton, style: .cancel, handler: {
            (action) in
    ...
    public func webView(_ webView: WKWebView, runJavaScriptConfirmPanelWithMessage message: String, initiatedByFrame frame: WKFrameInfo,
        ...
        onJsConfirm(frame: frame, message: message, result: {
            (result) -> Void in
            if result is FlutterError {
                print((result as! FlutterError).message ?? "")
                completionHandler(false)
    ...
    func createPromptDialog(message: String, defaultValue: String?, responseMessage: String?, confirmButtonTitle: String?, cancelButtonTitle: String?, value: String?, completionHandler: @escaping (String?) -> Void) {
        ...
        promptController.addAction(UIAlertAction(title: okButton, style: .default, handler: {
            (action) in
            if let v = value {
                completionHandler(v)
            }
        ...
        promptController.addAction(UIAlertAction(title: cancelButton, style: .cancel, handler: {
            (action) in
            completionHandler(nil)
        }))
        guard let presentingViewController = inAppBrowserDelegate != nil ? inAppBrowserDelegate as? InAppBrowserWebViewController : window?.rootViewController else {
    ...
    public func webView(_ webView: WKWebView, runJavaScriptTextInputPanelWithPrompt message: String, defaultText defaultValue: String?, initiatedByFrame frame: WKFrameInfo,
        ...
        onJsPrompt(frame: frame, message: message, defaultValue: defaultValue, result: {
            (result) -> Void in
            if result is FlutterError {
                print((result as! FlutterError).message ?? "")
                completionHandler(nil)
    ...
    public func webView(_ webView: WKWebView,
        ...
        channel?.invokeMethod("onCreateWindow", arguments: arguments, result: {
            (result) -> Void in
            if result is FlutterError {
                print((result as! FlutterError).message ?? "")
    ...
    public func webView(_ webView: WKWebView,
        ...
        shouldAllowDeprecatedTLS(challenge: challenge, result: {
            (result) -> Void in
            if result is FlutterError {
                print((result as! FlutterError).message ?? "")
                decisionHandler(false)
...
public func onCallJsHandler(handlerName: String, _callHandlerID: Int64, args: String) {
    ...
    channel?.invokeMethod("onCallJsHandler", arguments: arguments, result: {
        (result) -> Void in
        if result is FlutterError {
            print((result as! FlutterError).message ?? "")
        }
...
public func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
    if message.name.starts(with: "console") {
        ...
        if let webMessageListener = webMessageListeners.first(where: ({
            ($0.jsObjectName == jsObjectName)
        }))
        {
...
public func getHitTestResult(completionHandler: @escaping (HitTestResult) -> Void) {
    if configuration.preferences.javaScriptEnabled, let lastTouchLocation = lastTouchPoint {
        self.evaluateJavaScript("window.\(JAVASCRIPT_BRIDGE_NAME)._findElementsAtPoint(\(lastTouchLocation.x),\(lastTouchLocation.y))", completionHandler: {
            (value, error) in
            if error != nil {
...
public func requestFocusNodeHref(completionHandler: @escaping ([String: Any?]?, Error?) -> Void) {
    if configuration.preferences.javaScriptEnabled {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.15) {
            self.evaluateJavaScript("window.\(JAVASCRIPT_BRIDGE_NAME)._lastAnchorOrImageTouched", completionHandler: {
                (value, error) in
...
public func requestImageRef(completionHandler: @escaping ([String: Any?]?, Error?) -> Void) {
    if configuration.preferences.javaScriptEnabled {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.15) {
            self.evaluateJavaScript("window.\(JAVASCRIPT_BRIDGE_NAME)._lastImageTouched", completionHandler: {
                (value, error) in
...
public func isSecureContext(completionHandler: @escaping (_ isSecureContext: Bool) -> Void) {
    evaluateJavascript(source: "window.isSecureContext") {
        (isSecureContext) in
